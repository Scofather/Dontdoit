...
class BasicTransactionItemFormatter implements IGpcItemFormatter
{
	...
	public function format(IGpcItem $item): string
	{
		...
		$content .= str_pad((string) BigDecimal::of(
			$this->moneyFormatter->format($item->getAmount())
		)->multipliedBy(100)->toScale(0, RoundingMode::HALF_UP), 12, '0', STR_PAD_LEFT);
		...
		$content .= str_pad(
			(string) $this->isoCurrencies->numericCodeFor($item->getAmount()->getCurrency()),
			5, '0', STR_PAD_LEFT
		);
		$content .= $item->getDate()->format('dmy');
